...
function re_defaults_html($ERROR) {
	...
	if ($check) {
		$html .= "<br />\n";
		$html .= "<p class=\"error-alert\"> すでに登録されているか、登録出来ません。<p>\n";
		$html .= "<br />\n";
		$html .= "<form action=\"".$PHP_SELF."\" method=\"POST\">\n";
		$html .= "	<input type=\"submit\" class=\"btn-standard center\" value=\"アフィリエイトTOPに戻る\">\n";
		$html .= "</form>\n";
		return array($title,$html);
	}
...
function pc_defaults_html($af_num){
	...
	//	クリック数
	$sql  = "SELECT YEAR(click_time) AS year, COUNT(*) AS click FROM $afrefere_table" .	//	update yoshizawa SUM(*)→COUNT(*) 2013/12/10
			" WHERE af_num='$af_num'" .
...
//		$a_now_point += $now_point;
		...
		$html .= "		<div class=\"aff-point-content\" data-label=\"年\"><span class=\"aff-content1\"><input type=\"submit\" class=\"btn-standard\" name=\"year\" value=\"".$year."\">年</span></div>\n";
...
function pc_mon_html($af_num){
	...
	//	クリック数
	$sql  = "SELECT MONTH(click_time) AS mon, COUNT(*) AS click FROM $afrefere_table" .	//	update yoshizawa SUM(*)→COUNT(*) 2013/12/10
	...
	if ($first_year < $year) {
		...
		$first .= "			<input type=\"submit\" class=\"btn-standard\" value=\"&lt;&lt;\">\n";
		$first .= "		</form>\n";
	}
	...
	if ($now_year > $year) {
		...
		$next .= "			<input type=\"submit\" class=\"btn-standard\" value=\"&gt;&gt;\">\n";
		$next .= "		</form>\n";
	}
...
//		$a_now_point += $now_point;
		...
		$html .= "	<div class=\"box-row\">\n";
		$html .= "	<div class=\"aff-point-content\" data-label=\"月\"><span class=\"aff-content1\"><input type=\"submit\" class=\"btn-standard\" name=\"mon\" value=\"".$mon."\">月</span></div>\n";
		$html .= "	<div class=\"aff-point-content\" data-label=\"クリック数\">".$click_."件</div>\n";
		$html .= "	<div class=\"aff-point-content\" data-label=\"売上件数\">".$count_."件</div>\n";
		$html .= "	<div class=\"aff-point-content\" data-label=\"予定ポイント\">".$indent_point_."pt</div>\n";
		$html .= "	<div class=\"aff-point-content\" data-label=\"確定ポイント\">".$point_."pt</div>\n";
...
function pc_day_html($af_num){
	...
	//	クリック数
	$sql  = "SELECT DAY(click_time) AS day, COUNT(*) AS click FROM $afrefere_table" .	//	update yoshizawa SUM(*)→COUNT(*) 2013/12/10
...
//		$a_now_point += $now_point;
		...
		$html .= "	<div class=\"box-row\">\n";
		$html .= "	<div class=\"aff-point-content\" data-label=\"日\"><span class=\"aff-content1\">".$day."日</span></div>\n";
		$html .= "	<div class=\"aff-point-content\" data-label=\"クリック数\">".$click_."件</div>\n";
		$html .= "	<div class=\"aff-point-content\" data-label=\"売上件数\">".$count_."件</div>\n";
		$html .= "	<div class=\"aff-point-content\" data-label=\"予定ポイント\">".$indent_point_."pt</div>\n";
		$html .= "	<div class=\"aff-point-content\" data-label=\"確定ポイント\">".$point_."pt</div>\n";
...
function p_update_html($af_num,$ERROR){
	...
	//	変換履歴がなければメッセージ表示
	if ($count <= 0) {
		$html .= "<p class=\"error-alert\">今現在、変換申請履歴はございません。<br /><br /></p>\n";
		$html .= "<form action=\"".$PHP_SELF."\" method=\"POST\">\n";
		$html .= "	<input type=\"submit\" class=\"btn-standard center\" value=\"アフィリエイトTOPに戻る\">\n";
		$html .= "</form>\n";
		return array($title,$html);
	...
	while ($list = mysqli_fetch_array($result)) {
		...
		//	その他		→ 変換手続き完了待ち
			...
			$pay_day = "手続き完了待ち";
			$pay_price = 0;
			$submit = "<input type=\"submit\" class=\"btn-standard\"name=\"appli_num\" value=\"".$appli_num."\">";
		}
		...
		$html .= "	<div class=\"box-row\">\n";
		$html .= "	<div class=\"aff-point-content\" data-label=\"申請番号\">".$submit."</div>\n";
		$html .= "	<div class=\"aff-point-content\" data-label=\"申請日\">".$appli_day."</div>\n";
		$html .= "	<div class=\"aff-point-content\" data-label=\"変換種類\">".$CHENGE_TYPE2_L[$change_type]."</div>\n";
		$html .= "	<div class=\"aff-point-content\" data-label=\"変換申請ポイント\">".$af_point."pt</div>\n";
		$html .= "	<div class=\"aff-point-content\" data-label=\"変換ポイント・金額\">".$pay_price."</div>\n";
		$html .= "	<div class=\"aff-point-content\" data-label=\"状態・変換日\">".$pay_day."</div>\n";
		$html .= "	</div>\n";
	}
	...
	if (!$ERROR) {
		if ($change_type == 2) {
			if ($bank_num > 0) {
				...
				if (!$result = mysqli_query($conn_id,$sql)) { $ERROR[] = "振込先情報を更新出来ませんでした。"; }
			} else {
				$sql  = "INSERT INTO $bank_table" .
						" (af_num,bank_name,store_name,deposit,account_num,account_name)" .
						" VALUES('$af_num','$bank_name','$store_name','$deposit','$account_num','$account_name');";
			...
			$sql  = "UPDATE $bank_table SET" .
					" state='1'" .
					" WHERE bank_num='$bank_num';";
			if (!$result = mysqli_query($conn_id,$sql)) { $ERROR[] = "振込先情報を削除出来ませんでした。"; }
...
function p_cancel($af_num,&$ERROR){
	...
	if (!$ERROR) {
		$sql  = "UPDATE $application_table SET" .
				" state='2'" .
				" WHERE appli_num='$appli_num';";
		if (!$result = mysqli_query($conn_id,$sql)) { $ERROR[] = "変換情報を削除出来ませんでした。"; }
	}
