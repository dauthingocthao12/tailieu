...
function check_tokka($hinban) {
	...
	$tokka_flg = 0;
	if ($hinban) {
		$sql  = "SELECT COUNT(*) AS tokka_flg FROM category cate".
...
function cago_goods_html($KAGOS , $OPTIONS , $ERROR){
	...
	global	$TAX_ , $mochi_pri , $PHP_SELF , $SEBAN_P_N , $SENAME_P_N , $MUNEBAN_P_N , $PANT_P_N , $BACH_P_N , $BACH_N,
...
*/
	...
	if ($KAGOS) {
		...
		foreach ($KAGOS AS $val) {
			...
			$html .= "	<h4 class=\"box-title-form box-title-big text-center\">\n";
			$html .= "		オーダー内容\n";
			$html .= "	</h4>\n";
			...
			$html .= "								<input type=\"hidden\" name=\"hinban\" value=\"".$hinban_."\" />\n";
			$html .= "								<span>数量：</span><input type=\"text\" name=\"num\" value=\"".$num_."\" class=\"change-quantity\" /><br />\n";
	...
	if ($OPTIONS) {
		...
		foreach ($OPTIONS AS $val) {
			...
			$marking_html .= "<div class=\"box-outline primary-color\">\n";
			$marking_html .= "	<h4 class=\"box-title-form text-center\">オーダー内容</h4>\n";
			$marking_html .= "		<div class=\"box-row no-gap\">\n";
			...
			$marking_html .= "		    	<div class=\"box-column flex-item-gap box-content\">\n";
			$marking_html .= "					<div class=\"box-row fixed-row\">\n";
			$marking_html .= "						 <div class=\"product-detail-name\">商品名：　</div>\n";
			$marking_html .= "                       <span class=\"bold-text\">".$title_."</span>\n";
			$marking_html .= "					</div>\n";
			$marking_html .= "					<div class=\"breakdown\">\n";
			$marking_html .= "						<span class=\"flex-item-width25\">番号、ネームなど</span>\n";
			$marking_html .= "						<span class=\"text-right flex-item-width15\">単価</span>\n";
			$marking_html .= "						<span class=\"text-right\">数量</span>\n";
			$marking_html .= "						<span class=\"text-right flex-item-width10\">小計<span>\n";
			$marking_html .= "					</div>\n";
			...
			if ($hinban_ == "mochikomi") {
				...
				$marking_html .= "		<div class=\"breakdown product-details\">\n";
				$marking_html .= "			<span class=\"flex-item-width25\">持ち込み手数料</span>\n";
				$marking_html .= "			<span class=\"text-right flex-item-width15\">".$kakaku_." 円</span>\n";
				$marking_html .= "			<span class=\"text-right\">1</span>\n";
				$marking_html .= "			<span class=\"bold-text text-right flex-item-width10\">".$syoukei."円<span>\n";
				$marking_html .= "		</div>\n";
			}
			...
			if ($seban_l_) {
				...
				$marking_html .= "	<div class=\"breakdown product-details\">\n";
				$marking_html .= "		<span class=\"flex-item-width25\">背番号".$SEBAN_N[$seban_l_]."：<b class=\"bold-text marking-color-3\">".$seban_num_."</b></span>\n";
				$marking_html .= "		<span class=\"text-right flex-item-width15\">".$kakaku_." 円</span>\n";
				$marking_html .= "		<span class=\"text-right\">".$moji_num."</span>\n";
				$marking_html .= "		<span class=\"bold-text text-right flex-item-width10\">".$syoukei." 円</span>\n";
				$marking_html .= "	</div>\n";


				// 2022/12/21 追加 uenishi
				if(!$sename_l_  &&  !$muneban_l_ && !$pant_l_ && !$bach_l_) {
					$marking_html .= "			</div>\n";
					$marking_html .= "		</div>\n";
					$marking_html .= "	</div>\n";
					$marking_html .= "</div>\n";
			...
			if ($sename_l_) {
				...
				$marking_html .= "	<div class=\"breakdown product-details\">\n";
				$marking_html .= "		<span class=\"flex-item-width25\">背ネーム ".$SENAME_N[$sename_l_]."：<b class=\"bold-text marking-color-3\">".$sename_name_."</b></span>\n";
				$marking_html .= "		<span class=\"text-right flex-item-width15\">".$kakaku_." 円</span>\n";
				$marking_html .= "		<span class=\"text-right\">".$moji_num."</span>\n";
				$marking_html .= "		<span class=\"bold-text text-right flex-item-width10\">".$syoukei." 円</span>\n";
				$marking_html .= "	</div>\n";
			...
			if ($muneban_l_) {
				...
				$marking_html .= "	<div class=\"breakdown product-details \">\n";
				$marking_html .= "		<span class=\"flex-item-width25\">胸番号 ".$MUNEBAN_N[$muneban_l_]."：<b class=\"bold-text marking-color-3\">".$muneban_num_."</b></span>\n";
				$marking_html .= "		<span class=\"text-right flex-item-width15\">".$kakaku_." 円</span>\n";
				$marking_html .= "		<span class=\"text-right\">".$moji_num."</span>\n";
				$marking_html .= "		<span class=\"bold-text text-right flex-item-width10\">".$syoukei." 円</span>\n";
				$marking_html .= "	</div>\n";
			...
			if ($pant_l_) {
				...
				$marking_html .= "	<div class=\"breakdown product-details\">\n";
				$marking_html .= "		<span class=\"flex-item-width25\">パンツ番号 ".$PANT_N[$pant_l_]."：<b class=\"bold-text marking-color-3\">".$pant_num_."</b></span>\n";
				$marking_html .= "		<span class=\"text-right flex-item-width15\">".$kakaku_." 円</span>\n";
				$marking_html .= "		<span class=\"text-right\">".$moji_num."</span>\n";
				$marking_html .= "		<span class=\"bold-text text-right flex-item-width10\">".$syoukei." 円</span>\n";
				$marking_html .= "	</div>\n";
			...
			if ($bach_l_) {
				...
				$marking_html .= "				<div class=\"breakdown product-details\">\n";
				$marking_html .= "					<span class=\"flex-item-width25\">バッジ ：<b class=\"bold-text marking-color-3\">".$BACH_N[$bach_l_]."</b></span>\n";
				$marking_html .= "					<span class=\"text-right flex-item-width15\">".$kakaku_." 円</span>\n";
				$marking_html .= "					<span class=\"text-right\">1</span>\n";
				$marking_html .= "					<span class=\"bold-text text-right flex-item-width10\">".$syoukei." 円</span>\n";
				$marking_html .= "				</div>\n";
...
#	$msr = mb_convert_kana($msr, "asKV", "EUC-JP");
	...
	if (!preg_match("/^[0-9]*$/",$kei1)|| !preg_match("/^[0-9]*$/",$kei2)|| !preg_match("/^[0-9]*$/",$kei3)){
		$ERROR[] = "携帯番号が不正です。（半角数学で入力お願いします）";
	}
	if ($email == "") {
...
function cago_form_html($ERROR){	//	add ookawara 2014/01/06
	...
	if ($idpass && !$addr) {
		...
		if ($count >= 1) {
			...
			$list[22] = $list[22] ? $list[22] : ""; // key1列
			$list[23] = $list[23] ? $list[23] : ""; // key2列
			$list[24] = $list[24] ? $list[24] : ""; // key3列
			$list[25] = $list[25] ? $list[25] : ""; // email列
			list($kojin_num,$name_s,$name_n,$kana_s,$kana_n,$zip1,$zip2,$prf,$city,$add1,$add2,$tel1,$tel2,$tel3,$fax1,$fax2,$fax3,$email,$n,$n,$point,$saku,$kei1,$kei2,$kei3,$email2)=$list;
			$_SESSION['addr']=$list;
		...
		if ($idpass) {
			...
			$count = mysqli_num_rows($sql1);
			if ($count >= 1) {
				list($kojin_num,$point) = mysqli_fetch_array($sql1);
			} else {
				unset($idpass);
	...
	//	配達ご希望時間プルダウン
	$haitatu_html .= "<select name=\"haitatu\" class=\"input-full-length\">\n";
	...
	//	在庫切れの場合プルダウン
	$zaiko_html .= "<select name=\"zaiko\" class=\"input-full-length\">\n";
	...
	$shiharai_html .= "<select name=\"shiharai\" class=\"input-full-length\">\n";
...
function cago_kakunin_html($KAGOS , $OPTIONS , $ERROR , $mode){
	...
	global   $PRF_N , $HAITATU_N , $ZAIKO_N , $SHIHARAI_N , $TAX_ , $mochi_pri , $PHP_SELF , $SEBAN_P_N , $SENAME_P_N , $MUNEBAN_P_N ,
			 $PANT_P_N , $BACH_P_N , $BACH_N , $waribiki , $waribiki2 , $wa_member , $DISCOUNT_C , $DISCOUNT , $DISCOUNT , $SOURYOU_MURYOU , $P_RITU ,
	...
	if ($KAGOS) {
		...
		foreach ($KAGOS AS $val) {
			...
			$html .= "<div class=\"box-outline primary-color\">\n";
			$html .= "	<h4 class=\"box-title-form text-center\">\n";
			$html .= "		オーダー内容\n";
			$html .= "	</h4>\n";
			$html .= "		<div class=\"box-row\">\n";
			$html .= "			<div class=\"box-content box-item\">\n";
			...
			$html .= "						<div class=\"box-row fixed-row\">\n";
			$html .= "							<div class=\"product-detail-name\">単価：　</div>\n";
			$html .= "							<span class=\"bold-text item-vartical-middle\">".$kakaku_h." 円</span>\n";
			$html .= "						</div>\n";
			$html .= "					</div>\n";
			$html .= "					<div class=\"amount\">\n";
			$html .= "						<div>\n";
			$html .= "							数量：　<span class=\"bold-text\">".$num_."</span>\n";
			$html .= "						</div>\n";
			$html .= "						<div class=\"sub-total\">\n";
			$html .= "							小計：　<span class=\"bolder\">".$syoukei." 円</span>\n";
			$html .= "						</div>\n";
...
//	$tax = number_format($tax);
	...
	if ($shiharai == 1) {
		...
		$goukei_html .= "<div class=\"bold-text box-row flex-item-between\">\n";
		$goukei_html .= "	<span>代引手数料</span>\n";
		$goukei_html .= "	<span class=\"bold-text\">".$daibiki." 円</span>\n";
		$goukei_html .= "</div>\n";
	...
	//	(前払い)コンビニ払い	/*2013/09/02 yoshizawa プルダウンにコンビニ払いなし*/
		...
		$goukei_html .= "<div class=\"bold-text box-row flex-item-between\">\n";
		$goukei_html .= "	<span>請求書発行手数料</span>\n";
		$goukei_html .= "	<span class=\"bold-text\">".$con_tesu." 円</span>\n";
		$goukei_html .= "</div>\n";
	...
	//	後払い手数料追加 2010/12/10 add ookawara
		...
		$atobarai = atobarai;
		$goukei_html .= "<div class=\"bold-text box-row flex-item-between\">\n";
		$goukei_html .= "	<span>後払い決算手数料</span>\n";
		$goukei_html .= "	<span class=\"bold-text\">".$ato_tesu." 円</span>\n";
		$goukei_html .= "</div>\n";
	}
...
*/
	//	合計金額
	$goukei_html .= "<div class=\"text-border\"></div>\n";
	$goukei_html .= "<div class=\"bold-text box-row flex-item-between\">\n";
	$goukei_html .= "	<span class=\"bolder\">合計金額</span>\n";
	$goukei_html .= "	<span class=\"total-amount\">".$price_all." 円</span>\n";
	$goukei_html .= "</div>\n";
	...
	if ($kojin_num && $idpass && $k_point) {
		...
		$goukei_html .= "<div class=\"bold-text box-row flex-item-between\">\n";
		$goukei_html .= "	<span>獲得予定割引ポイント</span>\n";
		$goukei_html .= "	<span class=\"important\">".$k_point."pt</span>\n";
		$goukei_html .= "</div>\n";
	}

	// add uenishi 2022/12/26 合計金額エリアの閉じタグ
	$goukei_html .= "</div>\n";
	$goukei_html .= "</div>\n";
...
function cago_sent($KAGOS , $OPTIONS , $mode){
	...
	//	非会員処理
	if (!$idpass) {
		$sql =  "SELECT MAX(kojin_num) as max FROM ".T_KOJIN.
...
//echo($sql."<br>");
	...
	$sql1 = mysqli_query($conn_id,$sql);
	list($add_num) = mysqli_fetch_array($sql1);
	$add_num = $add_num + 1;
...
EOT;
			...
			//	追加
				...
				list($buy_n) = mysqli_fetch_array($sql1);
				$buy_n = $num_ + $buy_n;
...
//	}
	...
	//mysqli_close(DB);	//	del ookawara 2014/02/25
...
function zaiko_add($hinban_,$title_,$kakaku_) {
	...
	if ($check <= 0) {
		...
		$sql1 = mysqli_query($conn_id,$sql);
		list($zaiko_num) = mysqli_fetch_array($sql1);
