...
function set_data() {
	...
	$_SESSION['CHECK']['s_words_b'] = $s_words_b;
...
function default_html($ERROR) {
	...
	if ( $_SESSION['CHECK']['s_words']){
		$words_parts = $_SESSION['CHECK']['s_words'];
...
//print_r( $goods_name_candidacy);
		if(count($goods_name_candidacy) > 1){
			if(count($goods_name_candidacy) >= CANDIDACY_MAX && mb_strlen ($words_parts)<= CANDIDACY_CHARACTER_MIN){
				$INPUTS['CANDIDACY'] = "候補が多すぎます。文字数を増やしてください。";
			}else{
				$make_goods_name_btn ="";
				$candidacy_count=1;
				foreach($goods_name_candidacy as $candidacy){
					$make_goods_name_btn .="<button type='submit' value='{$candidacy}' class='candidacy-btn' name='s_words_b'><i class='fa fa-search' aria-hidden='true'></i>{$candidacy}</button>";
	...
	//候補ボタン
	if ( $_SESSION['CHECK']['s_words_b']){
		$INPUTS['WORDS'] = $_SESSION['CHECK']['s_words_b'];
		$where .= " AND ps.goods_name ='".$_SESSION['CHECK']['s_words_b']."'";
