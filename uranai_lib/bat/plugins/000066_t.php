 ...
 * @author Azet
 * @date 2016-03-11
 * @url http://www.htb.co.jp/ichimoni/oadigest/happy/
 * updated: okabe 2017/06/23
 */
class Zodiac000066 extends UranaiPlugin {
	...
	function run($CONTENTS) {		// add okabe 2017/06/23 $URL -> $CONTENTS
		...
		// monthは1~12の値
		// dayは1~31の値
		$date_pattern = "/<h3 class=\"happy\">{$now['year']}年{$now['month']}月{$now['day']}日/";
		// サイト毎に星座名の設定
		...
		foreach ($LINES AS $line) {
			...
			// rank 通常
			if ($date_check_ok) {
				$flag = preg_match("/<h4>(\d{1,2})位：(.*座)　/", $line, $MATCHES);
				if ($flag) {
					$rank_num = $MATCHES[1];
					$star_name = $MATCHES[2];
