		...
		foreach ($LINES AS $line) {
			...
			if ($date_check_ok) {
				if ($star_num == 0) {   //星座名の抽出
					...
					//各運勢の抽出
					if ($love_val < 0) {
						$flag = preg_match("/<span>恋愛運【(\d{1,2})】<\/span>/", $line, $MATCHES);
						if ($flag) {
							$love_val = intVal($MATCHES[1]) * 20;
						}
					}
					if ($money_val < 0) {
						$flag = preg_match("/<span>金運【(\d{1,2})】<\/span>/", $line, $MATCHES);
						if ($flag) {
							$money_val = intVal($MATCHES[1]) * 20;
						}
					}
					if ($work_val < 0) {
						$flag = preg_match("/<span>仕事運【(\d{1,2})】<\/span>/", $line, $MATCHES);
						if ($flag) {
							$work_val = intVal($MATCHES[1]) * 20;
						}
					}
					//echo "*".$star_num.": ".$love_val.", ".$money_val.", ".$work_val."\n";
