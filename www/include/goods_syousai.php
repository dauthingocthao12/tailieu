...
function goods_syousai($VALUE, $CHECK) {
	global	$waribiki, $TAX_, $b_file, $LOG_DIR, $DISCOUNT_C, $attention,
	...
	if ($g_name) {
		...
		if (count($IMG_L) > 0) {
			foreach ($IMG_L AS $key => $file_name) {
				if ($img_flg == 0) {
					$image_00 = "<a href=\"".$file_name."\" rel=\"lightbox[imagegroup]\" id=\"chage_link\" title=\"".$goods_name."\">".
								"<img src=\"".$file_name."\" width=\"300\" alt=\"".$goods_name."\" id=\"chage_img\" />".
								"</a>\n";
					$img_flg = 1;
				}

				$key += 1;
				$set_image = "image_0".$key;
				$$set_image   = "<a href=\"".$file_name."\" OnClick=\"ImgChange('".$file_name."');return false;\" rel=\"lightbox[imagegroup]\" class=\"thumbnail\" title=\"".$goods_name."\">";
				$$set_image  .= "<img src=\"".$file_name."\" width=\"142\" alt=\"".$goods_name."\" />";
				$$set_image  .= "</a>\n";
			}
		} else {
			$image_00 = "<img src=\"/".IMAGE."/FILLER.gif\" width=\"300\" alt=\"FUTBOLJERSEY.com\" />";
		}
		...
		if (!$waribiki && $sale_price > 0) {
			if ($wariritu) {
				$price_msg .= "<span class=\"bold font-size red\">".$wariritu."%割引</span><br />\n";
			}
			$price_msg .= "<span class=\"bold line-through\">".$price_."円(税込み)</span>";
			$price_msg .= "&nbsp;<span class=\"bold font-size red\">".$sale_price_."円(税込み)</span>\n";
		} else {
			$price_msg .= "<span class=\"bold font-size red\">".$price_."円(税込み)</span>\n";
		}
