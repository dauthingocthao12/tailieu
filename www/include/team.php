...
*/
	...
	//	背番号プルダウン
	$max = count($SEBAN_N);
	$seban_l_html .= "	<select name=\"seban_l\">\n";
	...
	//	背ネームプルダウン
	$max = count($SENAME_N);
	$sename_l_html .= "	<select name=\"sename_l\">\n";
	...
	//	胸番号プルダウン
	$max = count($MUNEBAN_N);
	$muneban_l_html .= "	<select name=\"muneban_l\">\n";
	...
	//	パンツ番号プルダウン
	$max = count($PANT_N);
	$pant_l_html .= "	<select name=\"pant_l\">\n";
	...
	foreach ($result as $val) {
		...
		if ($val) {
			...
			// 2022/12/6 レスポンシブ対応につき、コメントアウト uenishi
			// $html .= "<tr>\n";
			// $html .= "	<th colspan=\"2\">\n";
			// $html .= "		商品番号：" . $hinban . "<br />\n";
			// $html .= "		商 品 名：" . $title . "<br />\n";
		...
		for ($i = 1; $i <= $buy_n; $i++) {
			...
			// 2022/12/6 レスポンシブ対応につき、コメントアウト uenishi
			// $html .= "<tr>\n";
			// $html .= "	<th class=\"team_order_title\">".$i."着目</th>\n";
			// $html .= "	<td>\n";
			// $html .= "		番号: <input size=\"6\" type=\"text\" maxlength=\"3\" name=\"".$names_ban."\" value=\"".$check_names_ban."\">\n";
			// $html .= "		背ネーム: <input size=\"40\" type=\"text\" maxlength=\"20\" name=\"".$names_name."\" value=\"".$check_names_name."\">\n";
			// $html .= "	</td>\n";
			// $html .= "</tr>\n";
...
function team_order_confirmation($customer)
{
	...
	foreach ($result as $val) {
		...
		if ($val) {
			...
			// $goods_kakunin_html .= "	<th colspan=\"2\">\n";
			// $goods_kakunin_html .= "		商品番号：" . $hinban . "<br />\n";
			// $goods_kakunin_html .= "		商 品 名：" . $title . "<br />\n";
			// $goods_kakunin_html .= "		数    量：" . $buy_n . "\n";
			// $goods_kakunin_html .= "	</th>\n";
			// $goods_kakunin_html .= "</tr>\n";
		...
		for ($i = 1; $i <= $buy_n; $i++) {
			...
			$goods_kakunin_html .= "		<div class=\"names-on-back\">\n";
			$goods_kakunin_html .= 				$number_order;
			$goods_kakunin_html .= 				$name_order;
			$goods_kakunin_html .= "		</div>\n";
			$goods_kakunin_html .= "	</div>\n";
...
*/
	...
	if ($user_email) {
		...
		$sql1 = pg_query(DB, $sql);		//	SQLの実行
		$check = pg_num_rows($sql1);	//	pg_num_rows 行数を返す

		if ($check <= 0) {
			$idpass = "";
		} else {
			$list = pg_fetch_array($sql1);	//	pg_fetch_array 行を配列として取得する
