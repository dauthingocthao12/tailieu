...
*/
	...
	include_once(INCLUDE_DIR."team.php");
	...
	//	コンテンツ	
	if ($_POST['modes'] == "kaku") {		//	ボタンが押されたらエラーチェック

		team_order_check($customer, $ERROR);

	} elseif ($_POST['modes'] == "send") {	//	ボタンが押されたらメール送信

		team_order_send($customer, $ERROR);
	}

	if ($customer != "") {	//	商品情報があればTRUE

		if ($_POST['modes'] == "kaku" && count($ERROR) == 0){
			//	御見積商品確認
			$main_contents = team_order_confirmation($customer);

		} else {
			//	チームオーダー入力ページ
			$main_contents = team_order($customer, $ERROR);

		}

	} else {

		$main_contents = not_team_order();
